...
<div class="card shadow mb-4"><div class="card-body"><form method="POST" action="{{ route('users.invite.store') }}">@csrf
    ...
    <div class="custom-control custom-switch mb-4"><input id="is_admin" class="custom-control-input" name="is_admin" value="1" type="checkbox" @checked(old('is_admin'))><label class="custom-control-label" for="is_admin">{{ __('USERS_GRANT_ADMIN') }}</label></div>
